...
<div class="container">
    ...
    <div class="row">
        <div class="col-auto" style="margin-bottom: 5px;">
            <a href="{{ route('actualizar_padron_tgi') }}" class="btn btn-sm btn-primary" id="btnActualizarPadron">
        ...
        <form action="{{ route('padron_tgi') }}" method="GET" class="row mb-3" autocomplete="off">
            @csrf
            <!-- Input que filtra solo por FOLIO -->
                <div class="col-md-2">
                    <input type="text" name="search_folio" class="form-control" placeholder="Buscar por folio..." value="{{ request('search_folio') }}">
                </div>  


            <!-- Input que filtra por todo menos por FOLIO -->
            <div class="col-md-4">
                <input type="text" name="search_all" class="form-control" placeholder="Buscar por calle, partida, clave..." value="{{ request('search_all') }}">
            </div>
...
<div class="modal fade" id="modalEditar" tabindex="-1" aria-labelledby="modalEditarLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ route('padron_tgi.actualizar') }}" method="POST" autocomplete="off">
            @csrf
            @method('PUT')
