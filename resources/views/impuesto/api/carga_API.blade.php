...
@section('title', 'Cargar API')

@section('content')
    <div class="container-fluid">
        <h1>Flistros</h1>
        <div class="row">
            <form action="{{ route('carga_api') }}" method="GET" class="row gy-1 gx-2 align-items-center mb-2"
                autocomplete="off">
                <!-- Mes (2 dígitos) -->
                <div class="col-auto">
                    <input type="number" name="mes" class="form-control form-control-sm" placeholder="Mes"
                        value="{{ request('mes') }}" min="1" max="12" id="mes"
                        oninput="this.value = this.value.slice(0, 2)" style="width: 70px;">
                </div>

                <!-- Año (4 dígitos) -->
                <div class="col-auto">
                    <input type="number" name="anio" class="form-control form-control-sm" placeholder="Año"
                        value="{{ request('anio') }}" id="anio" oninput="this.value = this.value.slice(0, 4)"
                        style="width: 90px;">
                </div>
                ...
                <div class="col-auto">
                    <input type="number" name="folio" class="form-control form-control-sm" placeholder="Folio"
                        value="{{ request('folio') }}" maxlength="8" pattern="\d{1,8}" title="Máximo 8 números"
                        style="width: 120px;">
                </div>

                <!-- Partida / Clave (hasta 15 números) -->
                <div class="col-auto">
                    <input type="number" name="busqueda" class="form-control form-control-sm" placeholder="Partida"
                        value="{{ request('busqueda') }}" maxlength="18" pattern="\d{1,18}" title="Máximo 18 números"
                        style="width: 150px;">
                </div>
        ...
        <div class="d-flex justify-content-between align-items-end mb-3">
            {{-- Formulario original --}}
            <form method="POST" action="{{ route('cargarNuevoApi') }}" class="d-flex align-items-end gap-2">
                @csrf
            ...
            @if (request('anio') && request('mes'))
                <div class="d-flex gap-2  justify-content-end">
                    <div class="dropdown">
                        <button class="btn btn-sm btn-success dropdown-toggle" type="button" id="dropdownMenuOpciones"
                        ...
                        <ul class="dropdown-menu dropdown-menu-end shadow-sm" aria-labelledby="dropdownMenuOpciones">
                            <li>
                                <a class="dropdown-item d-flex align-items-center gap-2 text-success"
                                    href="{{ route('exportar_api_faltantes', ['anio' => request('anio'), 'mes' => request('mes')]) }}">
                                    <i class="bi bi-file-earmark-text"></i> Exportar faltantes
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item d-flex align-items-center gap-2 text-success" href="#"
                            ...
                            <li>
                                <a class="dropdown-item d-flex align-items-center gap-2 text-success"
                                    href="{{ route('exportar_broches_api', ['anio' => request('anio'), 'mes' => request('mes')]) }}"
                                    target="_blank">
                            ...
                            <li>
                                <a class="dropdown-item d-flex align-items-center gap-2 text-success"
                                    href="{{ route('exportar_broches_salas_api', ['anio' => request('anio'), 'mes' => request('mes')]) }}"
                                    target="_blank">
                            ...
                            <li>
                                <a class="dropdown-item d-flex align-items-center gap-2 text-success"
                                    href="{{ route('modificarBajado', ['anio' => request('anio'), 'mes' => request('mes')]) }}">
                    ...
                    <!-- Modal para armar broches -->
                    <div class="modal fade" id="modalArmarBroches" tabindex="-1"
                        aria-labelledby="modalArmarBrochesLabel" aria-hidden="true">
                                ...
                                <form method="POST">
                                    ...
                                    <div class="modal-body">
                                        <div class="row g-4">
                                            <!-- Columna TGI -->
                                            <div class="col-md-6">
                                                <h6 class="text-primary">API - Administra L</h6>
                                                ...
                                                <div class="input-group mb-2">
                                                    <input type="number" name="num_broches" id="cant_broches"
                                                        class="form-control form-control-sm" placeholder="Ej: 3">
                                                    <button type="button" class="btn btn-outline-primary btn-sm"
                                                        id="btn_calculaBroches">
                                            ...
                                            <!-- Columna Salas -->
                                            <div class="col-md-6">
                                                <h6 class="text-success">API - Salas</h6>
                                                <p class="fw-bold text-muted" id="monto_total_salas">Monto Total Salas: —
                                                </p>

                                                <label for="cant_broches_salas" class="form-label">Cantidad de
                                                    broches</label>
                                                <input type="text" id="cant_broches_salas"
                                                    class="form-control form-control-sm mb-3" value="1" readonly>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="modal-footer justify-content-between">
                                        <button type="submit" class="btn btn-primary btn-sm"
                                            ...
                                            <i class="bi bi-save"></i> Guardar Broches API
                                        </button>
                        ...
                        <tr>
                            ...
                            <td class="texto_tabla ">
                                @foreach ($compartidos as $compartido)
                                    <label
                                        class="{{ $compartido['estado'] !== 'ACTIVO' ? 'text-danger' : 'text-success' }}">
                                        {{ $compartido['folio'] }}
                                    </label>
                                    @if (!$loop->last)
                                        ,
                            ...
                            <td class=" texto_tabla">{{ $registro->partida_extraida ?? '' }}</td>

                            <td class="texto_tabla">{{ $registro->administra ?? '' }}</td>
                            <td class=" texto_tabla">{{ $registro->importe }}</td>
                            <td class=" texto_tabla">
                                {{ \Carbon\Carbon::parse($registro->fecha_vencimiento)->format('d/m/Y') }}</td>
                            <td class="texto_tabla">{{ $registro->bajado ?? '' }}</td>


                            <!-- elemina el registro -->
                            <td class="texto_tabla"tyle="position: relative;">
                                <div class="dropdown">
                                    <button class="btn btn-secondary btn-sm p-0" type="button"
                                        id="dropdownMenuButton{{ $registro->id }}" data-bs-toggle="dropdown"
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $registro->id }}">
                                        <li>
                                            <button type="button" class="dropdown-item text-primary"
                                                data-bs-toggle="modal" data-bs-target="#modalEstado{{ $registro->id }}">
                                                Modificar estado
                                            </button>
                                        </li>
                                        <li>
                                            <form action="{{ route('eliminarRegistro_api', $registro->id) }}"
                                                method="POST" class="d-inline form-eliminar" autocomplete="off">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <div class="modal fade" id="modalEstado{{ $registro->id }}" tabindex="-1"
                                    aria-labelledby="modalLabel{{ $registro->id }}" aria-hidden="true">
                                    <div class="modal-dialog modal-sm">
                                        <div class="modal-content">
                                            <form action="{{ route('modificarEstado', $registro->id) }}" method="POST"
                                                autocomplete="off">
                                                @csrf
                                                @method('PUT')
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="modalLabel{{ $registro->id }}">Modificar
                                                        estado</h5>
                                                ...
                                                <div class="modal-body">
                                                    <div class="form-group">
                                                        ...
                                                        <select name="estado" id="estado{{ $registro->id }}"
                                                            class="form-select">
    ...
    <div class="modal fade" id="modalNuevo" tabindex="-1" aria-labelledby="modalNuevoLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('cargarNuevoApiControllerManual_api') }}" method="POST" autocomplete="off">
                    @csrf
                    <div class="modal-body">
                        <div class="row g-2">
                            <div class="col-md-3">
                                <input type="text" name="folio" id="folio" class="form-control form-control-sm"
                                    placeholder="Folio">
                            </div>
                            <div class="col-md-5">
                        ...
                        <div class="row g-2 mt-3">
                            <div class="col-md-4">
                                <input type="text" name="partida" id="partida" class="form-control form-control-sm"
                                    placeholder="Partida">
                            </div>
                            {{-- <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <input type="text" name="administra" id="administra"
                                    class="form-control form-control-sm" placeholder="Administra">
                            </div>
                        </div>
                        ...
                        <div class="row g-2 mt-3">
                            ...
                            <div class="col-md-6">
                                <input type="text" name="importe" id="importe" class="form-control form-control-sm"
                                    placeholder="Importe">
                            </div>
                        </div>
                    </div>
...
@section('scripts')
    <script type="module" src="{{ asset('js/impuesto/Api/cargaAPI.js') }}"></script>
